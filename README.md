
# 🛸 Greetings, Traveler of the Multiverse! I'm Hayden 👋

> *"Don't Panic! Always carry a towel, double-check your SQL JOINs, and never push to production on a Friday."*

```ascii
+---------------------------------------------------------------------------+
| SYSTEM INITIALIZATION: Cyberdyne Systems Model T-800 (UFSC Branch)        |
| MISSION: Pursue Computer Science & unravel the laws of the Cosmos.        |
| STATUS: Mostly Harmless                                                   |
| VOIGHT-KAMPFF TEST STATUS: Passed                                         |
| PREFERRED FUEL: Caffeinated Dark Matter ☕                                 |
+---------------------------------------------------------------------------+
```
---

## 🤖 `cat ~/about_hayden.json`

```json
{
  "species": "Homo Technologicus (Voight-Kampff Test: PASSED)",
  "current_role": "Certified Space Plumber (fixing memory leaks & database pipelines)",
  "location": "Florianópolis, Earth-616 (Sector 2814, Dimension C-137)"
  "academic_base": "Universidade Federal de Santa Catarina (UFSC)",
  "passions": [
    "Computer Science & Software Lifecycle",
    "Database Optimization & Relational Algebra",
    "Observational Astronomy & Orbital Dynamics",
    "Classical Mechanics & Hardware Tinkering"
  ],
  "favorite_number": 42,
  "towel_status": "Always within reach",
  "space_complexity": "$O(n \\log n)$",
  "orbital_velocity_formula": "$v = \\sqrt{\\frac{GM}{r}}$",
  "mindset": "Curious by nature, mechanical by choice, code-driven by default."
}
```

---

## 🌌 Tech Stack & Subspace Frequencies

### ⚔️ Primary Weapons (Languages)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-003B57?style=for-the-badge&logo=sqlite&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)


### 🗄️ Galactic Archives & Tools
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

---


## 📈 Cyberdyne Telemetry

<p align="center">
  <img height="165" src="https://github-readme-streak-stats.herokuapp.com/?user=junhaumhayden&theme=synthwave&hide_border=true" alt="GitHub Streak" />
</p>


---

## 📡 Establish Subspace Communication

- 🛰️ **Subspace Frequency:** `hayden@ufsc.br`
- 📍 **Coordinates:** `27.5954° S, 48.5480° W` (Planet Earth / CTC - UFSC)
- 🌌 **Status:** Always open to discuss Computer Science, Astrophysics, or Mechanics!

<p align="center">
  <i>"May the Source Code be with you."</i> 🚀
</p>
